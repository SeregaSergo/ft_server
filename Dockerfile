...
RUN mv /app/nginx/bswag_site.com /etc/nginx/sites-available
...
RUN mv /app/wp-config.php /var/www/bswag_site.com/wordpress/
...
RUN rm phpMyAdmin-5.0.4-all-languages.tar.gz latest-ru_RU.tar.gz /etc/nginx/sites-enabled/default
